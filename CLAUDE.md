...
# Places — project conventions

## Development workflow

```
feature branch ──PR (squash)──▶ develop ──PR (merge commit)──▶ main ──▶ semantic-release
```

- Branch from `develop`. Never commit directly to `develop` or `main`.
- PR back into `develop`, **squash merge**. The PR title becomes the commit message, so it
  must be a valid Conventional Commit — CI checks it.
- Release: PR `develop` → `main`, merged as a **merge commit, not a squash**. Squashing here
  collapses every `feat:`/`fix:` into one message and semantic-release computes the wrong bump.
- Pushing `main` runs semantic-release: tag + GitHub Release with generated notes. There is no
  `CHANGELOG.md` file and no bot commit back to `main`.
